    ...
    st.session_state.database_soal = [
        {
            ...
            "kunci": ["市", "お知らせ", "に", "よれば", "、", "この", "道路", "は", "来週", "から", "工事", "が", "始まる", "という", "こと", "です", "。"],
            "soal": ["市", "お知らせ", "に", "よれば", "この", "道路", "は", "来週", "から", "工事", "が", "始まる", "という", "こと", "です", "、", "。"]
        }
    ]

# Inisialisasi State
if "index_soal" not in st.session_state: st.session_state.index_soal = 0
if "jawaban_user" not in st.session_state: st.session_state.jawaban_user = []
if "bank_kata" not in st.session_state: st.session_state.bank_kata = []
if "status_periksa" not in st.session_state: st.session_state.status_periksa = False

# State untuk Fitur Tukar Posisi Murni (Swap)
if "idx_kata_dipilih" not in st.session_state: st.session_state.idx_kata_dipilih = None
if "mode_tukar" not in st.session_state: st.session_state.mode_tukar = False
